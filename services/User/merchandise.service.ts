...
export const postMerchandiseService = async (body: IMerchandise, userId: string, userType: string, employerId: string, typeRole: string, userBranchId: string): Promise<IServiceLayerResponseMerchandise> => {
    try {
        if (userType === 'User') {
        ...
        const dataLayerResponse = await postMerchandiseData(body, userId, userType, employerId, typeRole);
...
export const postManyMerchandiseService = async (merchandises: IMerchandise[], userId: string, userType: string, employerId: string, typeRole: string, userBranchId: string): Promise<IServiceLayerResponseMerchandise> => {
    ...
    try {
        for (const merchandise of merchandises) {
            ...
            // Crear la mercancía
            const createdMerchandise = await postManyMerchandiseData(merchandise, userId, userType, employerId, typeRole);
...
export const getMerchandiseUserService = async (userId: string, userType: string): Promise<IServiceLayerResponseMerchandise> => {
    try {
        let dataLayerResponse;
        if (userType === 'User') {
            dataLayerResponse = await getMerchandiseByUserIdData(userId);
        }
...
export const getMerchandiseBranchService = async (idBranch: string, userId: string, userType: string): Promise<IServiceLayerResponseMerchandise> => {
    try {
        const hasPermission = await checkPermissionForBranchMerchandise(idBranch, userId, userType);
...
export const getMerchandiseService = async (idMerchandise: string, userId: string, userType: string): Promise<IServiceLayerResponseMerchandise> => {
    try {
        const hasPermission = await checkPermissionForMerchandise(idMerchandise, userId, userType);
...
export const putMerchandiseService = async (idMerchandise: string, body: IMerchandise, userId: string, userType: string): Promise<IServiceLayerResponseMerchandise> => {
    try {
        const hasPermission = await checkPermissionForMerchandise(idMerchandise, userId, userType);
        if (!hasPermission) throw new ServiceError(403, "No tienes permiso para actualizar esta mercancía");
        const updateMerchandise = await putMerchandiseData(idMerchandise, body, userId, userType);
...
export const putUpdateManyMerchandiseService = async (merchandises: IMerchandise[], userId: string, userType: string, employerId: string, typeRole: string, userBranchId: string): Promise<IServiceLayerResponseMerchandise> => {
    const uniqueMerchandises: IMerchandise[] = [];
    const duplicatedMerchandises: IMerchandise[] = [];
    ...
    try {
        for (const merchandise of merchandises) {
            ...
            const updatedMerchandise = await putUpdateManyMerchandiseData(merchandise, userId, userType,);
...
export const patchMerchandiseService = async (idMerchandise: string, body: any, userId: string, userType: string): Promise<IServiceLayerResponseMerchandise> => {
    try {
        const hasPermission = await checkPermissionForMerchandise(idMerchandise, userId, userType);
        if (!hasPermission) throw new ServiceError(403, "No tienes permiso para retirar del inventario esta mercancía");
        const updateMerchandise = await patchMerchandiseData(idMerchandise, body, userId, userType);
        if (!updateMerchandise) throw new ServiceError(404, "Mercancía no encontrada");
...
export const deleteMerchandiseService = async (idMerchandise: string, userId: string, userType: string): Promise<IServiceLayerResponseMerchandise> => {
    try {
        const hasPermission = await checkPermissionForMerchandise(idMerchandise, userId, userType);
