...
import { IAssets } from "../../../../types/UserPanel/03Inventories/01InventoryAssets/assets.types";
...
export const postAssetService = async (userId: string, body: IAssets): Promise<IServiceLayerResponseAssets> => {
    try {
        ...
        const dataLayerResponse = await postAssetData(userId, body);
...
export const getAssetByIdService = async (userId: string, idAssets: string): Promise<IServiceLayerResponseAssets> => {
    try {
        ...
        const assetFound = await getAssetByIdData(userId, idAssets);
...
export const getAssetBranchService = async (userId: string, idBranch: string): Promise<IServiceLayerResponseAssets> => {
    try {
        const hasPermission = await checkPermissionForBranchAssets(userId, idBranch);
...
export const getAssetsOffByBranchService = async (userId: string, idBranch: string): Promise<IServiceLayerResponseAssets> => {
    try {
        const dataLayerResponse = await getAssetsOffByBranchData(userId, idBranch);
        return { code: 200, result: dataLayerResponse };
    } catch (error) {
        if (error instanceof Error) {
            const customErrorMessage = error.message;
...
export const putUpdateManyAssetService = async (userId: string, typeRole: string, assets: IAssets[]): Promise<IServiceLayerResponseAssets> => {
    const uniqueAssets: IAssets[] = [];
    const duplicatedAssets: IAssets[] = [];
    try {
        for (const asset of assets) {
            ...
            const updatedAsset = await putUpdateManyAssetData(userId, asset);
            if (updatedAsset) {
...
export const patchAssetService = async (userId: string, idAssets: string, body: any): Promise<IServiceLayerResponseAssets> => {
    try {
        const hasPermission = await checkPermissionForAssets(userId, idAssets);
...
export const patchAddInventoryAssetService = async (userId: string, idAssets: string, body: any): Promise<IServiceLayerResponseAssets> => {
    try {
        const hasPermission = await checkPermissionForAssets(userId, idAssets);
        ...
        const updateAsset = await patchAddInventoryAssetData(userId, idAssets, body);
        if (!updateAsset) throw new ServiceError(404, "Equipo, máquina o herramienta no encontrado");
