...
export const paymentsAccountsPayableCompany = async (body: IAccountsBook, userId: string): Promise<any> => {
    if (body.creditDescription) {
        //Busca la CXP
        const accountPayableFound = await AccountsPayable.findOne({ where: { creditDescription: body.creditDescription, transactionCounterpartId: body.transactionCounterpartId, companyId: userId, stateAccount: 'Activo' } });
