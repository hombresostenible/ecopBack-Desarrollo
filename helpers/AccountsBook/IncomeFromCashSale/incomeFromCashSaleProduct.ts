...
export const incomeFromCashSaleProduct = async (item: IItemsAccountsBook, branchId: string, transactionType: string): Promise<any> => {
    ...
    if (transactionType === 'Ingreso') {
        if (item.quantity !== undefined) {
            ...
            if (productFound.inventory < item.quantity) {
                throw new ServiceError(400, `El inventario es insuficiente para la venta. Inventario actual: ${productFound.inventory}, Cantidad solicitada: ${item.quantity}`);
            }
            try {
