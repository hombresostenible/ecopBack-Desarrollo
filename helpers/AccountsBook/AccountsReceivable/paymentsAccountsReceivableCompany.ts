...
export const paymentsAccountsReceivableCompany = async (body: IAccountsBook, userId: string): Promise<any> => {
    if (body.creditDescription) {
        //Busca la CXC
        const accountReceivableFound = await AccountsReceivable.findOne({ where: { creditDescription: body.creditDescription, transactionCounterpartId: body.transactionCounterpartId, companyId: userId, stateAccount: 'Activo' } });
        ...
        if (accountReceivableFound) {
            ...
            if (body.totalValue >=  accountReceivableFound.currentBalance) {
                ...
                try {                            
                    ...
                    accountReceivableFound.creditPayments = accountReceivableFound.creditPayments.concat({                                              // Actualizar creditPayments usando concat para crear un nuevo array
